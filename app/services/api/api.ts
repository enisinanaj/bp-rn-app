...
const convertQuestion = (raw: any): QuestionSnapshot => {
  const id = raw.q.replace(" ", "") + "" + raw.category[0] + raw.difficulty + raw.correct_answer[0]
  return {
    id,
...
export class Api {
  ...
  async getQuestions(): Promise<Types.GetQuestionsResult> {
    // make the api call
    const response: ApiResponse<any> = await this.apisauce.get(this.config.url, { amount: API_PAGE_SIZE })
